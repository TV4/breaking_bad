defmodule BreakingBad.CircuitBreaker do
  ...
  def init(config) do
    if !Map.get(config, :interval_disabled, false) do
      :timer.send_interval(
        Application.get_env(:breaking_bad, :circuit_breaker_interval, 1000),
        config.name,
        :interval
      )
    end
  ...
  def handle_call(:unsubscribe, {caller, _ref}, circuit_breaker) do
    {:reply, :ok,
     Map.put(circuit_breaker, :listeners, List.delete(circuit_breaker.listeners, caller))}
  end
  ...
  def handle_cast(:melt, circuit_breaker = %__MODULE__{state: :ok}) do
    ...
    circuit_breaker =
      case do_melt(circuit_breaker) do
        ...
        circuit_breaker = %__MODULE__{state: :ok} ->
          circuit_breaker
      end
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, circuit_breaker) do
    ...
    {:noreply,
     Map.put(circuit_breaker, :monitored_refs, List.delete(circuit_breaker.monitored_refs, ref))}
  end
